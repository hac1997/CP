...
export const EditalCard: React.FC<EditalCardProps> = ({ edital }) => {
  ...
  const CardContent = () => (
    <>
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`${config.bgColor} ${config.textColor} px-3 py-1 rounded-full text-xs font-semibold ml-2 whitespace-nowrap flex-shrink-0`}>
          {config.label}
        </span>
      </div>
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm p-6 border-l-4 ${config.borderColor}`}>
      <CardContent />
    </div>
  );
};
